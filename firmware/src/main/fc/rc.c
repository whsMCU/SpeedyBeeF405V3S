...
#define RC_RX_RATE_MIN_US                       1000   // 0.950ms to fit 1kHz without an issue
...
{
    static uint32_t lastRxTimeUs;

    int32_t frameAgeUs;
    int32_t frameDeltaUs = rxGetFrameDelta(&frameAgeUs);
...
void rcSmoothingResetAccumulation(rcSmoothingFilter_t *smoothingData)
{
...
static bool rcSmoothingAccumulateSample(rcSmoothingFilter_t *smoothingData, int rxFrameTimeUs)
{

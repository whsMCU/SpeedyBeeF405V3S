...
#include "gps_rescue_multirotor.h"
...
gpsRescueConfig_t gpsRescueConfig;

void gpsRescueConfig_Init(void)
{
  gpsRescueConfig.landingAltitudeM = 4;
  gpsRescueConfig.maxRescueAngle = 45;
  gpsRescueConfig.returnAltitudeM = 30;
  gpsRescueConfig.descentDistanceM = 20;
  gpsRescueConfig.groundSpeedCmS = 750;
  gpsRescueConfig.yawP = 20;
  gpsRescueConfig.minSats = 8;
  gpsRescueConfig.velP = 8;
  gpsRescueConfig.velI = 40;
  gpsRescueConfig.velD = 12;
  gpsRescueConfig.minStartDistM = 15;
  gpsRescueConfig.sanityChecks = RESCUE_SANITY_FS_ONLY;
  gpsRescueConfig.allowArmingWithoutFix = false;
  gpsRescueConfig.useMag = GPS_RESCUE_USE_MAG;
  gpsRescueConfig.altitudeMode = GPS_RESCUE_ALT_MODE_MAX;
  gpsRescueConfig.ascendRate = 45;    // cm/s, for altitude corrections on ascent
  gpsRescueConfig.descendRate = 150;  // cm/s, minimum for descent and landing phase, or for descending if starting high ascent
  gpsRescueConfig.initialClimbM = 10;
  gpsRescueConfig.rollMix = 150;
  gpsRescueConfig.disarmThreshold = 30;
  gpsRescueConfig.pitchCutoffHz = 75;
  gpsRescueConfig.imuYawGain = 10;
...
static void setReturnAltitude(bool newGpsData)
{
    ...
    rescueState.intent.maxAltitudeCm = fmaxf(getEstimatedAltitudeCm(), rescueState.intent.maxAltitudeCm);

    if (newGpsData) {
        // set the target altitude to the current altitude.
        rescueState.intent.targetAltitudeCm = getEstimatedAltitudeCm();
        ...
        switch (gpsRescueConfig.altitudeMode) {
            ...
            case GPS_RESCUE_ALT_MODE_CURRENT:
                ...
                rescueState.intent.returnAltitudeCm = fmaxf(initialClimbCm, getEstimatedAltitudeCm() + initialClimbCm);
                break;
...
static void performSanityChecks(void)
{
    ...
    if (rescueState.phase == RESCUE_IDLE) {
        ...
        prevAltitudeCm = getEstimatedAltitudeCm();
    ...
    secondsLowSats += (!STATE(GPS_FIX) || (GpsNav.GPS_numSat < GPS_MIN_SAT_COUNT)) ? 1 : -1;
    ...
    const float actualAltitudeChange = getEstimatedAltitudeCm() - prevAltitudeCm;
...
static void sensorUpdate(bool newGpsData)
{
    ...
    const float altitudeCurrentCm = getEstimatedAltitudeCm();
    ...
    DEBUG_SET(DEBUG_GPS_RESCUE_HEADING, 1, GpsNav.GPS_ground_course);                // degrees * 10
    ...
    rescueState.sensor.healthy = GpsNav.flags.validEPE;//gpsIsHealthy();

    rescueState.sensor.directionToHome = GpsNav.GPS_directionToHome; // extern value from gps.c using current position relative to home
    ...
    rescueState.sensor.distanceToHomeCm = GpsNav.GPS_distanceToHomeCm;
    rescueState.sensor.distanceToHomeM = rescueState.sensor.distanceToHomeCm / 100.0f;
    rescueState.sensor.groundSpeedCmS = GpsNav.GPS_ground_course; // cm/s
...
static bool checkGPSRescueIsAvailable(void)
{
    ...
    if (!GpsNav.flags.validEPE || !STATE(GPS_FIX_HOME)) {
        return false;
    }
    ...
    secondsLowSats = constrain(secondsLowSats + ((GpsNav.GPS_numSat < GPS_MIN_SAT_COUNT) ? 1 : -1), 0, 2);
    if (secondsLowSats == 2) {
        lowsats = true;
        result = false;
    } else {
...
static void descend(bool newGpsData)
{
    if (newGpsData) {
        ...
        const float distanceToLandingAreaM = rescueState.sensor.distanceToHomeM - (0.5f * gpsRescueConfig.landingAltitudeM);
...
static bool isBelowLandingAltitude(void)
{
    return getEstimatedAltitudeCm() < 100.0f * gpsRescueConfig.landingAltitudeM;
}
...
// runs at gpsRescueTaskIntervalSeconds, and runs whether or not rescue is active
{
        ...
        if (!STATE(GPS_FIX_HOME)) {
            ...
            if (rescueState.sensor.distanceToHomeM < 5.0f && isBelowLandingAltitude()) {
                ...
                if (rescueState.sensor.distanceToHomeM < gpsRescueConfig.minStartDistM) {
                    ...
                    rescueState.intent.returnAltitudeCm = fmaxf(1000.0f, getEstimatedAltitudeCm() + (gpsRescueConfig.initialClimbM * 100.0f));
                ...
                returnAltitudeLow = getEstimatedAltitudeCm() < rescueState.intent.returnAltitudeCm;
    ...
    case RESCUE_ATTAIN_ALT:
        ...
        if (returnAltitudeLow == (getEstimatedAltitudeCm() < rescueState.intent.returnAltitudeCm)) {

...
{
    if (enabled) {
        osdConfig.enabled_stats |= (1 << statIndex);
    } else {
        osdConfig.enabled_stats &= ~(1 << statIndex);
    }
}
...
{
    if (enabled) {
        osdConfig.enabledWarnings |= (1 << warningIndex);
    } else {
        osdConfig.enabledWarnings &= ~(1 << warningIndex);
    }
}
...
static timeDelta_t osdShowArmed(void)
{
    ...
    if (false) {//isFlipOverAfterCrashActive()
...
static bool osdProcessStats1(timeUs_t currentTimeUs)
{
    ...
    if (ARMING_FLAG(ARMED)) {
        ...
        if (displayIsGrabbed(osdDisplayPort)) {
            ...
            if (true && osdStatsVisible) { //IS_RC_MODE_ACTIVE(BOXOSD)
                osdStatsVisible = false;
                displayClearScreen(osdDisplayPort, DISPLAY_CLEAR_NONE);
            } else if (false) {//!IS_RC_MODE_ACTIVE(BOXOSD)
                if (!osdStatsVisible) {
...
void osdProcessStats3()
{
    ...
    if (sensors(SENSOR_ACC)
       && (VISIBLE(osdElementConfig.item_pos[OSD_G_FORCE]) || osdStatGetState(OSD_STAT_MAX_G_FORCE))) {
        ...
        osdGForce = sqrtf(osdGForce) * bmi270.acc_1G_rec;
    }
#endif
}
...
void osdUpdate(timeUs_t currentTimeUs)
{
    ...
    case OSD_STATE_UPDATE_CANVAS:
        ...
        if (true) {//IS_RC_MODE_ACTIVE(BOXOSD)
            displayClearScreen(osdDisplayPort, DISPLAY_CLEAR_NONE);
            osdState = OSD_STATE_COMMIT;
            break;
        }
    ...
    if (true) {//!schedulerGetIgnoreTaskExecTime()

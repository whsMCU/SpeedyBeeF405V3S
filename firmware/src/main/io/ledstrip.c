...
ledStripStatusModeConfig_t ledStripStatusModeConfig;
void ledStripStatusModeConfig_Init(void)
{
...
bool parseLedStripConfig(int ledIndex, const char *config)
{
    ...
    ledConfig_t *ledConfig = &ledStripStatusModeConfig.ledConfigs[ledIndex];
...
bool parseColor(int index, const char *colorConfig)
{
    ...
    hsvColor_t *color = &ledStripStatusModeConfig.colors[index];
...
bool setModeColor(ledModeIndex_e modeIndex, int modeColorIndex, int colorIndex)
{
    ...
    if (modeIndex < LED_MODE_COUNT) {  // modeIndex_e is unsigned, so one-sided test is enough
        if (modeColorIndex < 0 || modeColorIndex >= LED_DIRECTION_COUNT)
            return false;
        ledStripStatusModeConfig.modeColors[modeIndex].color[modeColorIndex] = colorIndex;
    } else if (modeIndex == LED_SPECIAL) {
        if (modeColorIndex < 0 || modeColorIndex >= LED_SPECIAL_COLOR_COUNT)
            return false;
        ledStripStatusModeConfig.specialColors.color[modeColorIndex] = colorIndex;
    } else if (modeIndex == LED_AUX_CHANNEL) {
        if (modeColorIndex < 0 || modeColorIndex >= 1)
            return false;
        ledStripStatusModeConfig.ledstrip_aux_channel = colorIndex;
    } else {
        return false;
    }
    return true;
...
void setLedProfile(uint8_t profile)
{
    if (profile < LED_PROFILE_COUNT) {
      ledStripConfig.ledstrip_profile = profile;
    }
}
#endif

...
{
    uint32_t crsfCachedBaudrate = rtcBackupRegRead(PERSISTENT_OBJECT_SERIALRX_BAUD);
...
void crsfFrameFlightMode(sbuf_t *dst)
{
    ...
    if (FLIGHT_MODE(FAILSAFE_MODE)) {
        ...
        flightMode = "HOR";
    } else if (false) {//airmodeIsEnabled()
        flightMode = "AIR";
    }
...
void speedNegotiationProcess(timeUs_t currentTimeUs)
{
    ...
    } else if (false && crsfRxUseNegotiatedBaud()) {//!featureIsEnabled(FEATURE_TELEMETRY)

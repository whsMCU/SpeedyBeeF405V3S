...
void FAST_CODE scheduler(void)
{
  ...
  if (gyroEnabled) {
      ...
      if (schedLoopRemainingCycles < schedLoopStartCycles) {
          ...
          if (bmi270.gyroModeSPI != GYRO_EXTI_NO_INT) {
              ...
              if ((terminalGyroRateCount == 0)) {
                  terminalGyroRateCount = bmi270.detectedEXTI + GYRO_RATE_COUNT;
                  sampleRateStartCycles = nowCycles;
              }

              if (bmi270.detectedEXTI >= terminalGyroRateCount) {
              ...
              int32_t gyroSkew = cmpTimeCycles(nextTargetCycles, bmi270.gyroSyncEXTI) % desiredPeriodCycles;
              ...
              if ((terminalGyroLockCount == 0)) {
                  terminalGyroLockCount = bmi270.detectedEXTI + GYRO_LOCK_COUNT;
              }

              if (bmi270.detectedEXTI >= terminalGyroLockCount) {
